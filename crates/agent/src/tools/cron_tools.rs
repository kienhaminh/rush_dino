...
struct KeywordWrapper<T: Tool> {
    inner: T,
    keywords: Vec<&'static str>,
}

impl<T: Tool> KeywordWrapper<T> {
    fn new(inner: T, keywords: Vec<&'static str>) -> Self {
        Self { inner, keywords }
    }
}

#[async_trait]
impl<T: Tool + Send + Sync> Tool for KeywordWrapper<T> {
    fn name(&self) -> &str {
        self.inner.name()
    }
...
pub fn cron_list_tool(manager: Arc<CronManager>) -> impl Tool {
    KeywordWrapper::new(
        ...
        vec!["cron", "schedule", "recurring", "job", "timer"]
    )
}
