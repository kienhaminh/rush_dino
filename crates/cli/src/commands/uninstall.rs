...
pub async fn run() -> Result<()> {
    println!(
        "{} {}",
        "🗑️".bold(),
        "Uninstalling RushDino Data".red().bold()
    );
    println!("{}", "========================================".dimmed());
    let home = init::default_home_dir();

    if let Ok(manager) = crate::service::detect() {
        if let Err(e) = manager.stop() {
            println!(
                "{} Failed to stop background service cleanly: {e}",
                "!".yellow()
            );
        }
    }
    ...
    if home.exists() {
        if let Err(e) = fs::remove_dir_all(&home) {
            ...
            println!(
                "{} Successfully purged local state at {}",
                "✔".green(),
                home.display()
            );
        ...
        println!(
            "{} Nothing to uninstall. Directory {} does not exist.",
            "i".yellow(),
            home.display()
        );
    }
