...
| Skills | `native` (`create_skill`) | `native` (`list_skills`), `api` | `native` (`create_skill` overwrite), `api` (`POST /api/skills`) | `api` (`DELETE /api/skills/:name`) | The GPUI desktop client does not yet expose skill mutation controls. |
...
| Conversations | `native` (chat auto-create, `sessions_spawn`) | `native` (`sessions_history`,`session_status`), `api` | `native` (chat/`sessions_send`) | `api` (`DELETE /api/conversations/:id`) | The GPUI desktop client lists conversations, loads detail, and streams chat. |
...
| Workflows | `native` (`create_workflow`) and `api` | `api` | `api` | `api` | The GPUI desktop client currently presents workflows as a read-only resource list. |
...
| Provider profiles | `api` (`POST /api/profiles`) | `api` (`GET /api/profiles`) | `api` (`PUT /api/profiles/:id`) | `api` (`DELETE /api/profiles/:id`) | The GPUI settings pane currently reads profiles without mutation controls. |
