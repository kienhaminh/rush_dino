      ...
      <CardContent>
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {runtime.channels.map((channel) => (
            <div key={channel.id} className="border border-border/50 rounded-lg p-4 bg-muted/20 space-y-3">
              ...
              <div className="space-y-2">
                {channel.accounts.map((account) => (
                  <div key={account.accountId} className="border border-border/50 rounded-md p-3 bg-muted/10 space-y-1">
                    <div className="flex items-center justify-between">
                    ...
                    {account.lastError ? (
                      <p className="text-xs text-red-600">Last error: {account.lastError}</p>
                    ) : null}
                  </div>
                ))}
              </div>
