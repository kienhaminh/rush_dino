...
import { formatRelativeTimestamp } from "../components/common/format.ts";

import type { ChannelAccountSnapshot, NostrStatus } from "../components/common/types.ts";
...
export function renderNostrCard(params: {
  ...
  const summaryConfigured =
    nostr?.configured ?? primaryAccount?.configured ?? false;
  ...
  const summaryLastStartAt =
    nostr?.lastStartAt ?? primaryAccount?.lastStartAt ?? null;
  const summaryLastError =
    nostr?.lastError ?? primaryAccount?.lastError ?? null;
  const hasMultipleAccounts = nostrAccounts.length > 1;
  const showingForm =
    profileFormState !== null && profileFormState !== undefined;

  const renderAccountCard = (account: ChannelAccountSnapshot) => {
    const publicKey = (account as { publicKey?: string }).publicKey;
    const profile = (
      account as { profile?: { name?: string; displayName?: string } }
    ).profile;
    const displayName =
      profile?.displayName ??
      profile?.name ??
        ...
        ${hasAnyProfileData
          ? html`
              <div class="status-list">
                ...
                ${nip05
                  ? html`<div>
                      <span class="label">NIP-05</span><span>${nip05}</span>
                    </div>`
                  : nothing}
              </div>
            `
          : html`
              <div style="color: var(--text-muted); font-size: 13px">
                No profile set. Click "Edit Profile" to add your name, bio, and
                avatar.
              </div>
            `}
      </div>
