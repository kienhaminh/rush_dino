...
function renderChannel(
  ...
  switch (key) {
    ...
    case "nostr": {
      ...
      const profile =
        (primaryAccount as { profile?: NostrProfile | null } | undefined)
          ?.profile ?? null;
      const showForm =
        props.nostrProfileAccountId === accountId
          ? props.nostrProfileFormState
          : null;
...
function renderGenericChannelCard(
  ...
  const status = props.snapshot?.channels?.[key] as
    | Record<string, unknown>
    | undefined;
  const configured =
    typeof status?.configured === "boolean" ? status.configured : undefined;
  const running =
    typeof status?.running === "boolean" ? status.running : undefined;
  const connected =
    typeof status?.connected === "boolean" ? status.connected : undefined;
  const lastError =
    typeof status?.lastError === "string" ? status.lastError : undefined;
        ...
        : html`
            <div class="status-list" style="margin-top: 16px;">
              ...
              <div>
                ...
                <span
                  >${connected == null ? "n/a" : connected ? "Yes" : "No"}</span
                >
              </div>
            </div>
          `}
...
function resolveChannelMetaMap(
  ...
  return Object.fromEntries(
    snapshot.channelMeta.map((entry) => [entry.id, entry]),
  );
}
