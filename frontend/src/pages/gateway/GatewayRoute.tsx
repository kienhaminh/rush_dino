...
} from '../channels/ChannelsPage';
import { ChannelSettingsPage } from '../channels/ChannelSettingsPage';
...
function isUnregisteredAdapterError(error: unknown): boolean {
  return (
    error instanceof Error &&
    error.message.includes('gateway adapter') &&
    error.message.includes('is not registered')
  );
}

function adapterRegistrationGuidance(channel: string): string {
  return `${channel} settings were saved, but the adapter is not registered in the running server yet. Restart rushdino-server to load it.`;
}
...
export function GatewayRoute() {
  ...
  const load = useCallback(async (options?: { full?: boolean; showLoading?: boolean }) => {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      void load({ full: false, showLoading: false });
    }, 5000);
    return () => window.clearInterval(interval);
  }, [load]);

  const handleRestart = async (channelId: string) => {
    if (!window.confirm(`Restart the ${channelId} gateway adapter?`)) {
      return;
    }
    setRestartingChannelId(channelId);
    try {
      await restartGatewayAdapter(channelId);
      toast.success(`${channelId} adapter restart requested.`);
      await load();
    } catch (err) {
      if (isUnregisteredAdapterError(err)) {
        toast.error(adapterRegistrationGuidance(channelId));
      } else {
        toast.error(err instanceof Error ? err.message : 'Failed to restart gateway adapter.');
      }
    } finally {
  ...
  const handleResetGatewaySession = async (sessionId: string) => {
    if (
      !window.confirm(
        'Reset this gateway session? The next inbound channel message will open a new conversation.',
      )
    ) {
      return;
    }
    try {
      await resetGatewaySession(sessionId);
      toast.success('Gateway session reset.');
      await load();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to reset gateway session.');
    }
  };
  ...
  const handleChannelConfigAction = useCallback(
    async (channel: ChannelKey, patch: ChannelDetailConfigPatch, action: ChannelConfigAction) => {
          ...
          [channel]: {
            ...
            enabled:
              typeof requestedEnabled === 'boolean'
                ? requestedEnabled
                : config.gateway[channel].enabled,
      ...
      try {
        ...
        const shouldRestartAdapter =
          isPersistedGatewayChannel(channel) &&
          (action === 'connect' ||
            hasCredentialsPatch ||
            (typeof requestedEnabled === 'boolean' && requestedEnabled));
        let registrationGuidance: string | null = null;

        if (shouldRestartAdapter) {
          try {
            await restartGatewayAdapter(channel);
          } catch (err) {
            if (isUnregisteredAdapterError(err)) {
              registrationGuidance = adapterRegistrationGuidance(channel);
            } else {
              throw err;
            }
          }
        ...
        setLastSuccessAt(Date.now());
        setError(null);
        if (registrationGuidance) {
          toast.success(`${channel} configuration saved.`);
          toast.error(registrationGuidance);
  ...
  if (routeChannel && isChannelKey(routeChannel)) {
    ...
    const adapter = summary?.adapters?.find((item) => item.channelId === routeChannel) ?? null;
    const activity =
      summary?.channelActivity?.find((item) => item.channelId === routeChannel) ?? null;
    return (
      <ChannelSettingsPage
        key={routeChannel}
        ...
        restarting={restartingChannelId === routeChannel}
        ...
        onToggleEnabled={(channel, enabled) => {
          void handleChannelToggle(channel, enabled);
        }}
        onRestart={(channel) => {
          void handleRestart(channel);
        }}
  ...
  return (
    <GatewayPage
      ...
      loading={loading}
      error={error}
      restartingChannelId={restartingChannelId}
      onRefresh={() => void load({ full: true, showLoading: true })}
      onRestart={(channelId) => {
        void handleRestart(channelId);
      }}
      onChannelToggle={(channel, enabled) => {
        void handleChannelToggle(channel, enabled);
      }}
      onOpenChannelConfig={openChannelConfig}
      onResetGatewaySession={(sessionId) => {
        void handleResetGatewaySession(sessionId);
      }}
    />
  );
}
