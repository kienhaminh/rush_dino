...
import { clampText, formatRelativeTimestamp } from "../components/common/format.ts";
...
export function resolveExecApprovalsState(
  props: NodesProps,
): ExecApprovalsState {
  const form =
  ...
  const selectedScope = resolveExecApprovalsScope(
    props.execApprovalsSelectedAgent,
    agents,
  );
  const selectedAgent =
    selectedScope !== EXEC_APPROVALS_DEFAULT_SCOPE
      ? (((form?.agents ?? {})[selectedScope] as
          | Record<string, unknown>
          | undefined) ?? null)
      : null;
  const allowlist = Array.isArray(
    (selectedAgent as { allowlist?: unknown })?.allowlist,
  )
    ? ((selectedAgent as { allowlist?: ExecApprovalsAllowlistEntry[] })
        .allowlist ?? [])
    : [];
  return {
    ready,
  ...
  return html`
    <div class="row" style="margin-top: 12px; gap: 8px; flex-wrap: wrap;">
      ...
      <div class="row" style="gap: 8px; flex-wrap: wrap;">
        <button
          class="btn btn--sm ${state.selectedScope ===
        ...
        ${state.agents.map((agent) => {
          ...
          return html`
            <button
              class="btn btn--sm ${state.selectedScope === agent.id
                ? "active"
                : ""}"
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  const agentSecurity =
    typeof agent.security === "string" ? agent.security : undefined;
          ...
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option
                    value="__default__"
                    ?selected=${securityValue === "__default__"}
                  >
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        ${lastCommand
          ? html`<div class="list-sub mono">${lastCommand}</div>`
          : nothing}
        ${lastPath
          ? html`<div class="list-sub mono">${lastPath}</div>`
          : nothing}
      </div>
      <div class="list-meta">
        <label class="field">
...
function resolveExecApprovalsNodes(
  ...
  for (const node of nodes) {
    ...
    const supports = commands.some(
      (cmd) =>
        String(cmd) === "system.execApprovals.get" ||
        String(cmd) === "system.execApprovals.set",
    );
    if (!supports) {
      continue;
    }
