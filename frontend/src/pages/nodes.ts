...
import { formatRelativeTimestamp, formatList } from "../components/common/format.ts";
import {
  renderExecApprovals,
  resolveExecApprovalsState,
} from "./nodes-exec-approvals.ts";
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const age =
    typeof req.ts === "number" ? formatRelativeTimestamp(req.ts) : "n/a";
      ...
      <div class="list-main">
        ...
        ${tokens.length === 0
          ...
          : html`
              ...
              >
                ${tokens.map((token) =>
                  renderTokenRow(device.deviceId, token, props),
                )}
              </div>
            `}
      </div>
...
function renderTokenRow(
  ...
  return html`
    <div class="row" style="justify-content: space-between; gap: 8px;">
      ...
      <div
        class="row"
        style="justify-content: flex-end; gap: 6px; flex-wrap: wrap;"
      >
        <button
          class="btn btn--sm"
          @click=${() =>
      ...
      ${state.formMode === "raw"
        ? html`
            <div class="callout warn" style="margin-top: 12px">
              Switch the Config tab to <strong>Form</strong> mode to edit
              bindings here.
            </div>
          `
        : nothing}
      ...
      <div class="list-meta">
        <label class="field">
          ...
          >
            <option
              value="__default__"
              ?selected=${bindingValue === "__default__"}
            >
              Use default
            </option>
            ${state.nodes.map(
