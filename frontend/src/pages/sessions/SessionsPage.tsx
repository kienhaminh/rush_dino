...
function SessionRow({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`group relative rounded-[8px] px-3 py-2 w-full text-left cursor-pointer transition-all duration-150 border ${selected ? 'border-primary/20 bg-primary/[0.06]' : 'border-transparent hover:bg-muted/50'}`}
    >
      {/* Name + status dot */}
...
/* ─── InfoBadge ──────────────────────────────────────────────────────────── */
function InfoBadge({ label, active }: { label: string; active: boolean }) {
  return (
    ...
    <div className="border border-border rounded-lg p-3 bg-card space-y-1">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-1">
          {isReasoning && <InfoBadge label="REASONING MODEL" active />}
        </div>
      </div>
      ...
      <InfoRow label="Reasoning model">
        <InfoBadge label={isReasoning ? 'YES' : 'NO'} active={isReasoning} />
      </InfoRow>
      {agentConfig && (
        <>
...
/* ─── Session detail panel (keyed by session id to auto-reset tab on change) ─ */
function SessionDetail({
  ...
  // Starts fresh on every mount — key={selectedSessionId} in parent handles reset
    ...
    <div className="flex h-full w-full overflow-hidden">
      ...
      <div className="flex-1 min-w-0 h-full overflow-hidden flex flex-col">
        {selectedSessionId && session ? (
          /* key causes React to remount SessionDetail when the session changes,
             resetting activeTab back to 'overview' without a useEffect */
          <SessionDetail
