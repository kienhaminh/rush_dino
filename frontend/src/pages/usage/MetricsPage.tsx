...
import { MetricsProvidersTab } from './metrics-providers-tab';
...
type MetricsTab = 'usage' | 'providers';
...
// ─── UsageTab (existing content, extracted into this component) ───────────────
...
export function MetricsPage() {
  const [activeTab, setActiveTab] = useState<MetricsTab>('usage');

  return (
    <div className="flex-1 flex flex-col min-w-0 h-full bg-background">
      {/* Tab bar — acts as secondary nav below the main topbar */}
      <div className="shrink-0 flex items-center gap-1 px-4 border-b border-border bg-card h-11">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            id={`metrics-tab-${tab.id}`}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center justify-center gap-1.5 w-28 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${
              activeTab === tab.id
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        {activeTab === 'providers' && <MetricsProvidersTab />}
      </div>
    </div>
  );
}
