...
export function AgentIntelligence() {
  return (
    <section id="intelligence" className="relative py-32 border-t border-white/[0.06]">
      {/* Background glow */}
...
function AgentTeamCard() {
  const agents = [
    { name: "researcher", icon: "🔍", textClass: "text-[#22d3c8]",  model: "gpt-5.4" },
    { name: "coder",      icon: "💻", textClass: "text-[#3DBE8A]",  model: "claude-opus-4.6" },
    { name: "writer",     icon: "✍️",  textClass: "text-[#17C4D6]",  model: "kimi-2.5" },
    { name: "analyst",    icon: "📊", textClass: "text-[#F5C118]",  model: "o4-mini" },
  ];
  ...
  return (
    <div className="feature-card bg-[#080c10] p-7 flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-mono text-xs text-white/25 mb-2">01 / AGENT TEAM</div>
          <h3 className="font-mono font-bold text-white/85 text-lg leading-tight">
            Specialized Agent<br />Roster
          </h3>
        </div>
      ...
      <div className="flex flex-col gap-1.5">
        {agents.map((a) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className={`font-mono text-xs ${a.textClass}`}>{a.model}</span>
...
function WorkflowCard() {
  ...
  return (
    <div className="feature-card bg-[#080c10] p-7 flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-mono text-xs text-white/25 mb-2">02 / AGENT WORKFLOW</div>
          <h3 className="font-mono font-bold text-white/85 text-lg leading-tight">
            Multi-Step Workflow<br />Engine
          </h3>
        </div>
      ...
      <div className="code-block px-4 py-3">
        ...
        {steps.map((s) => (
          <div key={s.id} className="flex items-center gap-3 py-1">
            <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${STATUS_BG[s.status]} ${s.status === "running" ? "animate-pulse" : ""}`} />
            <span className="font-mono text-xs text-white/50 w-16 flex-shrink-0">{s.id}</span>
            <span className="font-mono text-xs text-white/60">{s.label}</span>
            <span className={`ml-auto font-mono text-xs ${STATUS_TEXT[s.status]}`}>{s.status}</span>
          </div>
        ))}
      </div>

      <div className="pt-3 border-t border-white/[0.06]">
        <span className="font-mono font-bold text-2xl text-[#17C4D6]">cron</span>
        <span className="font-mono text-xs text-white/25 ml-2 uppercase tracking-widest">
...
function KnowledgeGraphCard() {
  ...
  return (
    <div className="feature-card bg-[#080c10] p-7 flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-mono text-xs text-white/25 mb-2">03 / KNOWLEDGE GRAPH</div>
          <h3 className="font-mono font-bold text-white/85 text-lg leading-tight">
            Persistent Fact<br />Memory
          </h3>
        </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {facts.map((fact, i) => (
          <div key={i} className="flex items-start gap-2">
            <span className="font-mono text-xs text-[#3DBE8A]/60 flex-shrink-0 mt-0.5">▸</span>
            <span className="font-mono text-xs text-white/50 leading-snug">{fact}</span>
          </div>
        ))}
      </div>

      <div className="pt-3 border-t border-white/[0.06]">
        <span className="font-mono font-bold text-2xl text-[#3DBE8A]">SQLite</span>
        <span className="font-mono text-xs text-white/25 ml-2 uppercase tracking-widest">
...
/* ── Context Window Card ── */
const CTX_SEGMENTS = [
  { label: "system",   tokens: "1.2k",  barClass: "bg-[#22d3c8]/30",  textClass: "text-[#22d3c8]/80"  },
  { label: "history",  tokens: "11.4k", barClass: "bg-[#17C4D6]/30",  textClass: "text-[#17C4D6]/80"  },
  { label: "KG facts", tokens: "1.6k",  barClass: "bg-[#3DBE8A]/30",  textClass: "text-[#3DBE8A]/80"  },
];
...
function ContextWindowCard() {
  ...
  return (
    <div className="feature-card bg-[#080c10] p-7 flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-mono text-xs text-white/25 mb-2">04 / CONTEXT OPTIMIZATION</div>
          <h3 className="font-mono font-bold text-white/85 text-lg leading-tight">
            Context Window<br />Budget Control
          </h3>
        </div>
      ...
      <div className="code-block px-4 py-3 flex flex-col gap-3">
        <div className="flex justify-between font-mono text-xs text-white/30">
          ...
          <span>{used.toLocaleString()} / {(budget / 1000).toFixed(0)}k tokens</span>
        </div>
        <div className="h-1.5 bg-white/[0.06] rounded overflow-hidden">
          <div className={`h-full rounded w-[${pct}%] bg-gradient-to-r from-[#22d3c8] to-[#17C4D6]`} />
        </div>
        <div className="grid grid-cols-3 gap-2">
          {CTX_SEGMENTS.map((seg) => (
            <div key={seg.label} className="flex flex-col gap-0.5">
              <div className={`h-0.5 rounded ${seg.barClass}`} />
              <div className={`font-mono text-xs ${seg.textClass}`}>{seg.tokens}</div>
      ...
      <div className="pt-3 border-t border-white/[0.06]">
        <span className="font-mono font-bold text-2xl text-[#F5C118]">{pct}%</span>
        <span className="font-mono text-xs text-white/25 ml-2 uppercase tracking-widest">
    ...
    <div className="border border-white/[0.08] bg-[#0d1117] p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-white/25 tracking-widest">LIVE</span>
        </div>
      </div>
        ...
        <div className="flex items-start gap-0">
          {FLOW_STEPS.map((step, i) => {
            ...
            const labelClass = isDone
              ? "text-[#22d3c8]/45"
              : isActive
              ? step.accent ? "text-[#22d3c8]" : "text-white/85"
              : "text-white/20";
              ...
              <div key={i} className="flex items-start flex-shrink-0">
                ...
                <div className="flex flex-col items-center min-w-[110px]">
                  <div className="relative flex items-center justify-center w-5 h-5 mb-3">
                    ...
                    <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${dotClass}`} />
                  </div>
                  <div className={`font-mono text-xs leading-snug text-center px-1 max-w-[96px] transition-all duration-300 ${labelClass} ${isActive ? "-translate-y-px" : ""}`}>
                    {step.label}
                  </div>
                </div>
                ...
                {i < FLOW_STEPS.length - 1 && (
                  <div className="relative flex-shrink-0 mx-1 mt-[9px] w-7">
                    ...
                    <div className={`absolute top-0 left-0 h-px bg-gradient-to-r from-[#22d3c8]/60 to-[#22d3c8]/20 transition-all ease-out ${fillClass}`} />
                  </div>
                )}
              </div>
            );
